...
-- Hashed using: crypt('Prestige2026!', gen_salt('bf'))
-- =====================================================
...
insert into users (
  ...
  'active',
  'Executive',
  crypt('Prestige2026!', gen_salt('bf')),
...
-- Verify users (WITHOUT showing password_hash)
select
  user_id,
  email,
  phone,
...
-- Verify password hashing works (test password verification)
-- This should return TRUE for all users:
select
  email,
  role,
  (password_hash = crypt('Prestige2026!', password_hash)) as password_valid
from users;
